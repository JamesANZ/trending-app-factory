  ...
  if (form) {
    form.addEventListener("submit", function (e) {
      ...
      if (email) {
        ...
        showNotification(
          "🎉 Successfully subscribed! Welcome to the College Football Hub family!",
          "success",
        );
        this.reset();
...
function initScrollAnimations() {
  ...
  const animatedElements = document.querySelectorAll(
    ".trending-card, .team-card, .player-card, .fantasy-card, .news-card",
  );
...
function showTrendingDetails(topic) {
  const details = {
    "Deion Sanders' Impact on Colorado": {
      description:
        'The "Prime Effect" continues to transform college football as Deion Sanders leads Colorado into the 2025 season with unprecedented attention and expectations.',
      stats: [
    ...
    "College Football Season Kickoff": {
      description:
        "The 2025 season officially begins with major matchups, new conference alignments, and the expanded playoff format that's changing everything.",
      stats: [
        "🏆 12-Team Playoff",
        "🌍 New Conference Maps",
    ...
    "Heisman Trophy Race": {
      description:
        "Early Heisman favorites emerge as quarterbacks and skill players showcase their talents in what promises to be the most competitive race in years.",
...
function showPlayerStats(playerName, playerPosition) {
  const playerData = {
    "Quinn Ewers": {
      ...
      highlights:
        "Heisman favorite with elite arm talent and championship aspirations",
    },
    "Travis Hunter": {
...
function updateLiveScores() {
  ...
  scores.forEach((score) => {
    ...
    console.log(
      `Live: ${score.team1} ${score.score1} - ${score.team2} ${score.score2} (${score.quarter})`,
    );
  });
}
